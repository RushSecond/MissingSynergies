...
class IrradiateBuff(Buff):
    ...
    def on_advance(self):
        ...
        for unit in self.owner.level.get_units_in_ball(Point(self.owner.x, self.owner.y), radius):
            ...
            unit.deal_damage(2, Tags.Arcane, self.spell)
            unit.deal_damage(2, Tags.Poison, self.spell)
            effects_left -= 1
...
class Electrolysis(Upgrade):
    ...
    def get_description(self):
        return ("Whenever a [poisoned] enemy takes [lightning] damage, it has a chance to consume all [poison] and shoot out toxic lightning bolts at random enemies in a [{radius}_tile:radius] burst, including itself.\n"
                "The trigger chance is equal to the enemy's percentage of missing HP. The number of bolts is equal to the consumed [poison] duration divided by 20, rounded up.\n"
                "Each bolt [acidifies:poison] the target, reducing [poison] resistance by 100, or deals [{damage}_lightning:lightning] damage if the target is already [acidified:poison]. Either way, it then deals [{damage}_poison:poison] damage.").format(**self.fmt_dict())
    
    def on_damaged(self, evt):
        if not are_hostile(self.owner, evt.unit) or evt.damage_type != Tags.Lightning:
            return
        unit = evt.unit
        ...
        for _ in range(math.ceil(poison.turns_left/20)):
            ...
            if not targets:
                return
            self.owner.level.queue_spell(self.bolt(unit, random.choice(targets), damage))
...
class WithdrawalBuff(Buff):
    ...
    def on_advance(self):
        self.owner.deal_damage(2, Tags.Poison, self.source)
        self.owner.deal_damage(2, Tags.Arcane, self.source)
...
class PsychedelicSporesSpell(Spell):
    ...
    def get_description(self):
        return ("Summon a giant psychedelic mushboom with [{minion_health}_HP:minion_health].\n"
                ...
                "An enemy with delirium suffers a [-3:damage] penalty to all spell damage per stack.\n"
                "When delirium is removed, it applies a stack of withdrawal, inflicting [2_poison:poison] and [2_arcane:arcane] damage per turn per stack.").format(**self.fmt_dict())
...
class DeathMetalSpell(Spell):
    ...
    def get_description(self):
        return ("Channel this spell up to 10 turns to summon a metalhead near you and increase the remaining durations of all metalheads by [1_turn:minion_duration].\n"
...
class CosmicStasisBuff(FrozenBuff):
    ...
    def shoot(self, target):
        ...
        for point in path:
            self.owner.level.deal_damage(point.x, point.y, 0, Tags.Ice, self.spell)
            yield 

        self.owner.level.deal_damage(target.x, target.y, self.spell.get_stat('damage'), Tags.Ice, self.spell)
...
class CosmicStasisSpell(Spell):
    ...
    def get_description(self):
        return ("[Freeze] yourself for [{duration}_turns:duration].\n"
                "For the duration, you gain [100_ice:ice], [100_fire:fire], [100_physical:physical], and [100_arcane:arcane] resistance, and each turn you fire an ice bolt at an enemy in line of sight dealing [{damage}_ice:ice] damage.\n"
...
class SecretsOfBlood(Upgrade):
    ...
    def get_description(self):
        return ("Each turn, each stack of bloodrage on each ally has a 50% chance to have its remaining duration increased by [1_turn:duration]. Bloodrage is typically generated by [demon] minions with \"blood\" in their names.\n"
                "Whenever one of your minions deals damage to an enemy with an attack, each bloodrage stack on the attacker inflicts a stack of bleed on the target for the same duration, which deals [physical] damage per turn equal to the bloodrage stack's damage bonus and then has a 50% chance to be removed.\n"
    ...
    def on_damaged(self, evt):
        if not are_hostile(evt.unit, self.owner) or not evt.source or not evt.source.owner or evt.source.owner is self.owner or are_hostile(evt.source.owner, self.owner) or not isinstance(evt.source, Spell):
            return
...
class CarcinizationPassive(Buff):
    ...
    def on_init(self):
        self.buff_type = BUFF_TYPE_PASSIVE
        self.color = Tags.Nature.color
        self.description = "When lifetime expires, evolve into a crab."
        self.owner_triggers[EventOnDeath] = self.on_death
    
    def on_death(self, evt):
        if self.owner.turns_to_death == 0:
...
class CarcinizationSpell(Spell):
    ...
    def get_description(self):
        return ("For [{duration}_turns:duration], whenever you summon a temporary minion, it gains the ability to evolve into a crab with the same max HP when its lifetime expires.\n"
...
class BloodFodder(Upgrade):
    ...
    def on_advance(self):
        if all([u.team == TEAM_PLAYER for u in self.owner.level.units]):
            return
        max_num = (self.owner.max_hp - self.owner.cur_hp)//10
    ...
    def get_description(self):
        return ("You are accompanied by a number of hemogoblins equal to your missing HP divided by 10, which are replenished to the maxumum number each turn and summoned onto random tiles.\n"
...
class RealityFeintBuff(Soulbound):
    ...
    def unfeint(self):
        self.owner.deal_damage(-self.owner.max_hp if self.ultimate else -self.damage, Tags.Arcane, self.spell)
...
class EyeBleach(Upgrade):

    def on_init(self):
        self.name = "Eye Bleach"
...
class ThornShot(Upgrade):
    ...
    def can_redeal(self, target, source, damage_type, already_checked):
        if not isinstance(source, Spell) or not source.range:
            return False
        for buff in source.owner.buffs:
            if not isinstance(buff, Thorns):
                continue
            if not is_immune(target, buff, buff.dtype, already_checked):
                return True
        return False
